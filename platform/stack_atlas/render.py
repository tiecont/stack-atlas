...
from stack_atlas.redirects import render_legacy_redirects
...
def render_site(domains, categories, paths, articles, output: Path, site: dict):
    ...
    css = (ROOT / "platform/assets/styles/site.css").read_text(encoding="utf-8")
    tokens_css = (ROOT / "platform/assets/styles/tokens.css").read_text(encoding="utf-8")
    javascript = (ROOT / "platform/assets/scripts/site.js").read_text(encoding="utf-8")
    api_client = (ROOT / "platform/assets/scripts/api-client.js").read_text(encoding="utf-8")
    progress_store = (ROOT / "platform/assets/scripts/progress-store.js").read_text(encoding="utf-8")
    path_context = (ROOT / "platform/assets/scripts/path-context.js").read_text(encoding="utf-8")
    favicon = (ROOT / "platform/assets/icons/favicon.svg").read_text(encoding="utf-8")
    write("assets/site.css", "/* GENERATED FILE — edit platform/assets/styles/site.css */\n" + css)
    write("assets/tokens.css", "/* GENERATED FILE — edit platform/assets/styles/tokens.css */\n" + tokens_css)
    write("assets/api-client.js", "// GENERATED FILE — edit platform/assets/scripts/api-client.js\n" + api_client)
    write("assets/progress-store.js", "// GENERATED FILE — edit platform/assets/scripts/progress-store.js\n" + progress_store)
    write("assets/path-context.js", "// GENERATED FILE — edit platform/assets/scripts/path-context.js\n" + path_context)
    write("assets/site.js", "// GENERATED FILE — edit platform/assets/scripts/site.js\n" + javascript.replace("__BASE_PATH__", json.dumps(base_path)))
    ...
    for share_dir in ("labs", "examples/atlas-demo-api"):
        if (ROOT / share_dir).is_dir():
            shutil.copytree(ROOT / share_dir, output / share_dir, dirs_exist_ok=True)
    version_matrix = ROOT / "tests/kubernetes/version-matrix.yaml"
    if version_matrix.is_file():
        write_file(output / "tests/kubernetes/version-matrix.yaml", version_matrix.read_text(encoding="utf-8"))
    render_legacy_redirects(articles, paths, output, site)
