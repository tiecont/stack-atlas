...
def make_catalog():
    ...
    categories = read_yaml(CONTENT / "categories.yaml")
    paths = [read_yaml(path) for path in sorted((CONTENT / "paths").glob("*.yaml"))]
    ...
    errors = []
    explicit = []
    for file in sorted((CONTENT / "articles").rglob("article.yaml")):
        ...
        record["source"] = (file.parent / "article.html").relative_to(ROOT).as_posix()
