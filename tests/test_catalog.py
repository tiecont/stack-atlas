...
class CatalogValidationTests(unittest.TestCase):
    def load_fixture(self, root: Path, *, duplicate_id=False, unknown_domain=False, unknown_path_article=False,
        ...
        (content / "articles/demo").mkdir(parents=True)
        (content / "paths").mkdir()
        (content / "domains.json").write_text(json.dumps([{
            "id": "demo", "title": "Demo", "description": "Demo domain"
        }]))
        (content / "categories.json").write_text(json.dumps([{"id": "basics", "title": "Basics"}]))
        (content / "articles/demo/one.html").write_text("<h2>One</h2>")
        article = {
            ...
            "url": "/articles/demo/one/", "source": "content/articles/demo/one.html",
        }
        (content / "articles/demo/one.json").write_text(json.dumps(article))
        ...
        (content / "paths/demo-path.json").write_text(json.dumps(path))
        if duplicate_id:
            duplicate = dict(article, title="Duplicate one", url="/articles/demo/duplicate/")
            (content / "articles/demo/two.json").write_text(json.dumps(duplicate))
