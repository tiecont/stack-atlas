...
: >"$calls"
if CONTEXT=kind-production "$root/scripts/ci/kubernetes/assert_lab_context.sh" 2>/dev/null; then
...
: >"$calls"
if FAKE_CLUSTER_CONTEXT=kind-production "$root/scripts/ci/kubernetes/assert_lab_context.sh" 2>/dev/null; then
