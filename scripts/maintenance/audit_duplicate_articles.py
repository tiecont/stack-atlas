...
def load_articles() -> list[dict]:
    articles = []
    for path in sorted(CONTENT.rglob("article.yaml")):
        article = yaml.safe_load(path.read_text(encoding="utf-8"))
...
def main() -> int:
    ...
    lines = [
        ...
        "",
        "Generated by `python3 scripts/maintenance/audit_duplicate_articles.py`. Candidate generation is a review aid; it never merges articles or changes IDs, routes, content, or path membership.",
        "",
        "## Classification key",
        "",
